...
void ShowGameInstructions() {
	...
	int title_size = 80;
	int text_size = 40;
	COLORREF bg_color = RGB(39, 82, 91);
	COLORREF text_color = RGB(191, 235, 218);
	...
	outtextxy(screen_width / 2 - textwidth(_T("操作介绍")) / 2, 50, _T("操作介绍"));

	// 文本
	int text_height = 150;
	settextstyle(text_size, 0, font_style);
	outtextxy(screen_width / 2 - 300, text_height, _T("方向键：W A S D"));
	outtextxy(screen_width / 2 - 300, text_height + 60, _T("攻击键：J"));
	outtextxy(screen_width / 2 - 300, text_height + 120, _T("退出结算界面：Enter"));

	//设置鼠标点击的矩形区域
	RECT exit_rect;
	LPCTSTR exit_text = _T("退出");
	exit_rect.left = screen_width - 100;
	exit_rect.right = exit_rect.left + textwidth(exit_text);
	exit_rect.top = 20;
	exit_rect.bottom = exit_rect.top + textheight(exit_text);

	outtextxy(exit_rect.left, exit_rect.top, exit_text);

	ExMessage mess;
	while (true) {
		if (peekmessage(&mess, EM_MOUSE)) {
			if (mess.message == WM_LBUTTONDOWN && PointInRect(mess.x, mess.y, exit_rect)) {
				return;
			}
		}
	}
...
bool Welcome() {
	...
	RECT exit_rect;
	LPCTSTR exit_text = _T("操作介绍");
	exit_rect.left = title_x - textwidth(exit_text) / 2;
	exit_rect.right = title_x + textwidth(exit_text) / 2;
	exit_rect.top = screen_height / 5 * 3.5;
	exit_rect.bottom = exit_rect.top + textheight(exit_text);
	...
	outtextxy(exit_rect.left, exit_rect.top, exit_text);	//绘制操作介绍文字
	...
	while (1) {
		if (peekmessage(&mess,EM_MOUSE)) {
			if (mess.message == WM_LBUTTONDOWN) {
				//cout << "mess.x, mess.y:" << mess.x << "," << mess.y << endl;
				...
				else if (PointInRect(mess.x, mess.y, exit_rect)) {
...
int ReadScore() {
	string filePath = "./max_score/score.txt";
	...
	if (file.is_open()) {
		...
		if (file >> score) {
			...
			//cout << "Failed to read the score from the file." << endl;
			score = -1; // 设置默认值
		}
...
bool UpdateScore(int new_score) {
	string filePath = "./max_score/score.txt";
	ifstream fileIn(filePath);

	if (fileIn.is_open()) {
		int current_score;
		if (fileIn >> current_score) {
			fileIn.close();
			...
			if (new_score > current_score) {
				...
				if (fileOut.is_open()) {
					fileOut << new_score;
					fileOut.close();
					//cout << "New score updated: " << new_score << endl;
					return true;
				}
				else {
					//cout << "Failed to update the score. Unable to open the file: " << filePath << endl;
					return false;
				}
			}
			else {
				//cout << "New score is not higher than the current score." << endl;
				return true;
			}
		}
		else {
...
void ShowResult(unsigned int box, unsigned int smallbox, int base_score) {
	...
	const int text_x2 = text_x1 + screen_width / 3; // 总分和历史最高分的横坐标
	...
	const int text_y4 = text_y3 + text_size * 2; // 历史最高分的纵坐标
	...
	int max_score = -1;
	max_score = ReadScore();
	TCHAR max_score_text[20];
	_stprintf_s(max_score_text, _T("历史最高分：%u"), max_score);
	outtextxy(text_x2, text_y4, max_score_text);
...
int main() {
	...
	while (is_playing) {
		is_start_play = Welcome();
		if(is_start_play)is_playing = Play();
	}
	system("pause");
	closegraph();
	return 0;
