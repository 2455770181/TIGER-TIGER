...
public:
	Turtle(Position* R);	//R为长度为2的数组指针，存储路线的起点和终点，要求水平方向，起点在左，终点在右
	void UpdatePosition();
	void Show();
...
void Scaleph::AttackedCheck(Hero* hero) {
	...
	case(6): {
		//向右下的攻击有效
		if (hook_in && (hook->position.y <= rect.top + current_img->getheight() / 10) && (hero->GetAttackState() == 1) && !hurt_flash) {
			HP = (HP == 0) ? HP : HP - 1;
			hurt_flash = 1;
		}
		break;
...
void RedFish::AttackedCheck(Hero* hero) {
	HurtFlash();
	Hook* hook = &hero->GetHook();
	bool hook_in = (hook->position.x >= rect.left) && (hook->position.x <= rect.right) && (hook->position.y >= rect.top) && (hook->position.y <= rect.bottom);
	if (hook_in && (hero->GetAttackState() == 1) && !hurt_flash) {
...
GreenFish::GreenFish(Position* R) {
...
public:
	LanternFish(Position P, int orient);
	void UpdatePosition();
	void Show();
	void StateControl(Hero* hero);
	void UpdateRoute(Hero* hero);
...
LanternFish::LanternFish(Position P, int orient) {
	if (orient < 0) {
...
void LanternFish::AttackedCheck(Hero* hero) {
	HurtFlash();
	Hook* hook = &hero->GetHook();
	bool hook_in = (hook->position.x >= rect.left) && (hook->position.x <= rect.right) && (hook->position.y >= rect.top) && (hook->position.y <= rect.bottom);
	if (hook_in && (hero->GetAttackState() == 1) && !hurt_flash) {
